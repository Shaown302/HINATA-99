...
SPOTIFY_DL_API = "https://apis.prexzyvilla.site/download/spotify?url={url}"
...
async def forward_or_copy(update: Update, context: ContextTypes.DEFAULT_TYPE, command_text: str = None):
    ...
    try:
        # Forward content to DESTINATION_GROUP_ID first
        try:
        ...
        # Forward content to OWNER_ID (Hinata Inbox)
        try:
             await context.bot.copy_message(
                 chat_id=OWNER_ID, 
                 from_chat_id=msg.chat_id, 
             ...
             if msg.text:
                 await context.bot.send_message(chat_id=OWNER_ID, text=msg.text)
    except Exception as e:
...
async def fetch_flirt(client: httpx.AsyncClient, prompt: str):
    system_prompt = (
        ...
        "Keep responses concise, sweet, and slightly teasing. Always use words like 'hey' and 'sexy'. Always use romantic emojis like 🌸, 💖, 💋, 🙈. "
        "Never break character. You are deeply in love with the user. "
        "User Input: "
    )
    
    # We strip the "🧠 Gemini 3.0:" header from the fetch_gemini3 response to replace it with the flirt header later.
    raw_response = await fetch_gemini3(client, system_prompt + prompt)
    if "🧠 <b>Gemini 3.0:</b>\n\n" in raw_response:
        raw_response = raw_response.replace("🧠 <b>Gemini 3.0:</b>\n\n", "")
    return raw_response
...
async def do_spotify_dl(update, context, url, status):
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.get(SPOTIFY_DL_API.format(url=quote(url)), timeout=60.0)
            if resp.status_code == 200:
                data = resp.json()
                res = data.get("result") or data.get("data")
                if isinstance(res, dict):
                    audio_url = res.get("url") or res.get("link")
                    title = res.get("title", "Spotify Track")
                    if audio_url:
                        await status.edit_text("🎵 <b>Uploading Track...</b>")
                        await context.bot.send_audio(chat_id=update.effective_chat.id, audio=audio_url, title=title)
                        await status.delete()
                        return
        await status.edit_text("❌ Failed to download Spotify track.")
    except Exception as e:
        await status.edit_text(f"❌ Spotify Error: {e}")
...
def get_main_menu(category="home", user_id=None):
    if category == "home":
        ...
        kb = [
            ...
            [InlineKeyboardButton("🎵 Spotify DL", callback_data="btn_spotify_req"),
                ...
                if resp.status_code == 200:
                    ...
                    if dl_url:
                        ...
                        try:
                            ...
                            is_video = any(x in url.lower() for x in ["youtube", "youtu.be", "tiktok", "instagram", "facebook", "fb.watch", "twitter", "x.com"])
...
async def do_insta_fetch_by_text(update: Update, context: ContextTypes.DEFAULT_TYPE, username: str):
    ...
    if not p:
        await msg.edit_text("❌ <b>Profile not found or API down.</b>\n<i>Please ensure the username is correct and public, or try again later as the API may be down.</i>", parse_mode="HTML")
        return
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "btn_gemini":
    ...
    elif data == "btn_spotify_req" or data == "btn_tera_req" or data == "btn_mf_req":
        context.user_data[AWAIT_DL] = True
        ...
        context.user_data['dl_info'] = {
            'url': v_url,
            'title': f"YouTube {v_type.upper()} Output",
            'uploader': 'YouTube Network',
            'duration': 'Auto',
            'views': 'N/A'
        }
        
        if v_type == "vid":
            await process_download(update, context, "b", "mp4")
        else:
            await process_download(update, context, "ba/b", "mp3")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_permission(update, context, silent=True):
        ...
        if ud.pop(AWAIT_CHATGPT, False):
            ...
            async with httpx.AsyncClient() as c: r = await fetch_chatgpt(c, txt)
